...
class PriorityOrderManager:
    ...
    async def send_to_priority_drivers(
        ...
        logger.info(f"⭐ Замовлення #{order_id}: знайдено {len(priority_drivers)} пріоритетних водіїв (з увімкненим пріоритетом)")
        ...
        # Відправити ВСІМ пріоритетним водіям (не топ-5, а всім з увімкненим пріоритетом)
        sent_count = 0
        ...
        if sent_count > 0:
            ...
            logger.info(f"⏰ Таймер запущено для замовлення #{order_id} на 30 секунд")
            return True
        else:
    ...
    async def start_priority_timer(
        ...
        """Запустити таймер на 30 секунд для пріоритетного замовлення"""
...
def _build_priority_message(order_id: int, order_details: dict) -> str:
    ...
    return (
        ...
        f"<b>🎯 Це замовлення доступне тільки водіям з пріоритетом!</b>\n"
        f"<i>⏰ У вас є 30 секунд для прийняття рішення</i>\n\n"
        f"👤 Клієнт: {order_details.get('name', 'Не вказано')}\n"
        ...
        f"💬 Коментар: {order_details.get('comment', 'Немає')}\n"
        f"{route_link}\n\n"
        f"✅ <b>Прийміть</b> - замовлення буде закріплено за вами\n"
        f"❌ <b>Відхиліть</b> - інші пріоритетні водії все ще бачать замовлення\n\n"
        f"⏰ Якщо всі відхилять або пройде 30 секунд - замовлення автоматично з'явиться в загальній групі."
    )
